...
public class Exercise_CheckBoxItemEvent extends JFrame{
	...
	private String[] names = {"사과", "배", "체리"};  // 체크박스 버튼 이름 배열
	...
	public Exercise_CheckBoxItemEvent() {
		...
		c.setLayout(new FlowLayout());  // flowlayou 사용
		sum = 0;
			...
			fruits[i].addItemListener(new ItemListener() {  // 아이템리스너 핸들러 등록
				...
				public void itemStateChanged(ItemEvent e) {  // 아이템이 선택될 때
					if (e.getStateChange() == ItemEvent.SELECTED) {  // 선택이 됐으면
						...
						else if (e.getItem().equals(fruits[2])) {  // 그 이름이 체리
							sum += 2000;							// 2000을 더하렴
						}
					}
					else {  // 체크가 해제되면
						...
						else if (e.getItem().equals(fruits[2])) {  // 체리는 2000빼셈
							sum -= 2000;
						}
					}
...
//		c.add(cherry);
//		// 체크박스에 이벤트 걸기
//		apple.addItemListener(new ItemListener() {  // 메소드가 한개 밖에 없으면 adapter 없음
//			
//			@Override
