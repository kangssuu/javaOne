	...
	public void startMemberProgram() {
		...
		finish: while (true) {
			...
			case 2:
				...
				if (index != -1) {
					mOne = this.modifyMember(mOne); // 데이터가 있을때 수정 할 정보 입력 받음
				} // 수정된 데이터 (mOne)
				controller.modifyMember(index, mOne); // 데이터 수정하기
				break;
			case 3:
				name = this.inputMemberName(); // 삭제할 데이터의 이름을 입력 받음
				index = controller.findIndexByName(name); // 인덱스를 구한 이유 => subMember메소드가 인덱스 값을 받기 때문에.
				if (index != -1) {
			...
			case 4:
				...
				break;
			case 5:
				this.printAllMembers(controller.allMembermList());
				break;
			case 6:
				break finish;
